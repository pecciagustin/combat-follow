...
const PARTNERS_KEY = 'cf:partners'
const partnerMembersKey = (token) => `cf:partner-members:${token}`
...
export async function recordLogin(redis, payload) {
  ...
  const record = {
    ...
    maxFighters,
    features,
    // Partner scoping: preserved across logins. `scopedEventId` (when set) locks
    // the account to a single event; `source` records how they joined.
    scopedEventId: existing?.scopedEventId ?? null,
...
// Admin: lift or set an account's event scope. Passing null/'' clears it,
// turning a partner-scoped athlete into a full multi-event account.
export async function setUserScope(redis, email, scopedEventId) {
  const key = String(email).toLowerCase()
  const user = await getUser(redis, key)
  if (!user) throw new Error('User not found')
  user.scopedEventId = scopedEventId || null
  await redis.hset(USERS_KEY, { [key]: JSON.stringify(user) })
  return user
}

// ── Tech-partner affiliate links ──────────────────────────
// A partner is an event that paid to offer Combat Follow to its athletes. Its
// link pre-loads the event, auto-approves whoever signs in, and scopes them to
// that one event. Stored in the hash `cf:partners` keyed by an unguessable token.
...
export async function getPartner(redis, token) {
  return parseRecord(await redis.hget(PARTNERS_KEY, String(token || '').trim()))
}

export async function listPartners(redis) {
  const all = await redis.hgetall(PARTNERS_KEY)
  if (!all) return []
  return Object.values(all)
    .map(parseRecord)
    .filter(Boolean)
...
export async function createPartner(redis, { eventName, matchlistUrl, maxFighters, note, createdBy } = {}) {
  const name = String(eventName || '').trim().slice(0, 120)
  const url = String(matchlistUrl || '').trim().slice(0, 500)
  if (!name) throw new Error('Falta el nombre del evento')
  if (!url) throw new Error('Falta la match list (URL)')
  let token = generateToken()
  for (let i = 0; i < 5 && (await redis.hget(PARTNERS_KEY, token)); i++) token = generateToken()
  const record = {
    token,
    eventName: name,
    matchlistUrl: url,
    eventId: `partner:${token}`,
    maxFighters: normalizeMaxFighters(maxFighters, 'team'),
    active: true,
  ...
  await redis.hset(PARTNERS_KEY, { [token]: JSON.stringify(record) })
  return record
}

export async function setPartnerActive(redis, token, active) {
  const partner = await getPartner(redis, token)
  if (!partner) throw new Error('Partner no encontrado')
  partner.active = !!active
  await redis.hset(PARTNERS_KEY, { [partner.token]: JSON.stringify(partner) })
  return partner
}

// Auto-approve + scope the account to the partner's event. No-op (returns null)
// if the token is unknown or inactive. The admin is never scoped/altered.
export async function joinPartner(redis, email, token) {
  const key = String(email).toLowerCase()
  const partner = await getPartner(redis, token)
  if (!partner || !partner.active) return null
  const user = await getUser(redis, key)
  if (!user) return null // recordLogin runs first, so this should exist
  const isAdmin = key === ADMIN_EMAIL

  if (!isAdmin) {
    user.status = 'approved'
    user.scopedEventId = partner.eventId
    user.maxFighters = normalizeMaxFighters(partner.maxFighters, user.tier || 'team')
    user.source = { type: 'partner', token: partner.token, eventName: partner.eventName, at: Date.now() }
    await redis.hset(USERS_KEY, { [key]: JSON.stringify(user) })
    // Count each athlete once.
    const added = await redis.sadd(partnerMembersKey(partner.token), key)
    if (added) {
      partner.joinedCount = (partner.joinedCount || 0) + 1
      await redis.hset(PARTNERS_KEY, { [partner.token]: JSON.stringify(partner) })
    }
  }
  return { user: { ...user, isAdmin }, partner }
}

export async function listPartnerMembers(redis, token) {
  const emails = (await redis.smembers(partnerMembersKey(token))) || []
  const members = []
  for (const e of emails) {
    const u = await getUser(redis, e)
    if (u) members.push({ email: u.email, name: u.name, lastSeen: u.lastSeen, status: u.status })
  }
  return members.sort((a, b) => (b.lastSeen || 0) - (a.lastSeen || 0))
}
