...
// Apply a partner code's tier/quota/features to an account and approve it.
export async function applyTierToUser(redis, email, { tier, maxFighters, features } = {}) {
  const key = String(email).toLowerCase()
  const user = await getUser(redis, key)
  if (!user) throw new Error('User not found')
  const nextTier = VALID_TIERS.includes(tier) ? tier : user.tier || 'fighter'
  user.tier = nextTier
  user.maxFighters = normalizeMaxFighters(maxFighters ?? TIER_DEFAULTS[nextTier], nextTier)
  user.features = normalizeFeatures(features ?? user.features)
  if (key !== ADMIN_EMAIL) user.status = 'approved' // a code is an invitation
  await redis.hset(USERS_KEY, { [key]: JSON.stringify(user) })
  return user
}
